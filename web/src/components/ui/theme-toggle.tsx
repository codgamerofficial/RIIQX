...
export function ModeToggle() {
    ...
    const isDark = theme === "electronics";

    return (
        <button
            onClick={() => setTheme(isDark ? "fashion" : "electronics")}
            ...
            title={isDark ? "Switch to Fashion Mode" : "Switch to Electronics Mode"}
        >
            {/* Background Gradient Animation */}
            <div
                className={`absolute inset-0 transition-opacity duration-500 ${isDark ? 'opacity-0' : 'opacity-100'}`}
                style={{ background: 'linear-gradient(to right, #60a5fa, #3bf6ff)' }} // Sky blue for day (Fashion)
            />
            <div
                className={`absolute inset-0 transition-opacity duration-500 ${isDark ? 'opacity-100' : 'opacity-0'}`}
                style={{ background: 'linear-gradient(to right, #0f172a, #312e81)' }} // Deep space for night (Electronics)
            />
            ...
            >
                <div className="absolute inset-0 flex items-center justify-center">
                    {/* Icons swapped/updated if needed, but Sun/Moon works for general Light/Dark representation */}
                    <motion.div
                        initial={false}
