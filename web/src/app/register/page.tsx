...
export default function RegisterPage() {
    ...
    const passwordStrength = (password: string) => {
        ...
        return { strength: "STRONG", color: "text-[#B4F000]" };
    };
    ...
    if (pageLoading) {
        return <div className="min-h-screen bg-[#050505] flex items-center justify-center text-[#B4F000] font-mono text-xs tracking-widest uppercase animate-pulse">Establishing Secure Uplink...</div>;
    }
    ...
    return (
        <div className="min-h-screen bg-[#050505] flex overflow-hidden">
            ...
            <div className="w-full lg:w-1/2 relative flex items-center justify-center p-6 sm:p-12">
                ...
                {/* Decorative Glows */}
                <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] bg-[#B4F000]/5 rounded-full blur-[120px] pointer-events-none" />
                            ...
                            >
                                ...
                                <div className="mb-8">
                                    <h1 className="text-5xl md:text-6xl font-black text-white uppercase tracking-tighter font-[family-name:var(--font-oswald)] italic mb-3 leading-[0.9]">
                                        Create <br /><span className="text-[#B4F000]">Profile</span>
                                    </h1>
                                    ...
                                    {error && (
                                        ...
                                        >
                                            <p className="text-red-500 text-[10px] font-mono uppercase tracking-widest flex items-center gap-2">
                                                <Terminal className="w-3 h-3" /> Error: {error}
                                            </p>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                                ...
                                <form onSubmit={handleRegister} className="space-y-6">
                                    ...
                                    <div className="group">
                                        ...
                                        <div className="relative">
                                            <input
                                                ...
                                                className="w-full bg-white/5 border-b border-white/10 px-0 py-3 pl-4 text-white placeholder:text-white/10 focus:outline-none focus:border-[#B4F000] focus:bg-white/10 transition-all duration-300 font-mono text-sm tracking-wider"
                                            />
                                            <User className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/20 group-focus-within:text-[#B4F000] transition-colors" />
                                        </div>
                                    </div>
                                        ...
                                        <div className="group">
                                            ...
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className="w-full bg-white/5 border-b border-white/10 px-0 py-3 pl-4 text-white placeholder:text-white/10 focus:outline-none focus:border-[#B4F000] focus:bg-white/10 transition-all duration-300 font-mono text-sm tracking-wider"
                                                />
                                                <Phone className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/20 group-focus-within:text-[#B4F000] transition-colors" />
                                            </div>
                                        </div>
                                    ...
                                    <div className="group">
                                        ...
                                        {formData.password && (
                                            <p className={`text-[9px] mt-2 font-mono uppercase tracking-widest flex items-center gap-1 ${strength.color}`}>
                                                <Terminal className="w-3 h-3" /> Security Level: {strength.strength}
                                            </p>
                                        )}
                                    </div>

                                    {/* Terms */}
                                    <div className="group cursor-pointer" onClick={() => setAgreedToTerms(!agreedToTerms)}>
                                        <div className="flex items-start gap-3">
                                            <div className={`w-4 h-4 mt-0.5 border border-white/20 flex items-center justify-center transition-colors ${agreedToTerms ? 'bg-[#B4F000] border-[#B4F000]' : 'bg-transparent'}`}>
                                                {agreedToTerms && <Check className="w-3 h-3 text-black" />}
                                            </div>
                                            <span className="text-[10px] uppercase font-bold text-white/40 leading-relaxed select-none group-hover:text-white transition-colors">
                                                I Accept The <Link href="/terms" className="text-white hover:text-[#B4F000] underline decoration-white/30">Terms</Link> & <Link href="/privacy" className="text-white hover:text-[#B4F000] underline decoration-white/30">Privacy Protocol</Link>
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div className="mt-8 text-center">
                                    <Link href="/login" className="text-white/30 hover:text-white text-[10px] font-mono uppercase tracking-[0.2em] transition-colors group">
                                        Have Clearance? <span className="text-[#B4F000] underline decoration-[#B4F000]/30 underline-offset-4 group-hover:decoration-white font-bold ml-2">Login</span>
                                    </Link>
                                </div>
                            </motion.div>
                        )}
