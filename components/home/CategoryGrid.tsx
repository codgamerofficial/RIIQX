  ...
  return (
    <section className="max-w-7xl mx-auto px-6 space-y-8">
      ...
      <motion.div
        ...
        className="flex items-center justify-between border-b border-glass-border-subtle pb-4"
      >
        ...
        <Link href="/collections/all">
          <span className="font-mono text-xs text-riiqxText-secondary hover:text-white flex items-center gap-1 transition-colors">
            VIEW ALL CATEGORIES <ArrowUpRight className="w-3.5 h-3.5" />
          </span>
        </Link>
      </motion.div>

      {/* Cyber Category Tiles Grid with Framer Motion Stagger */}
      <motion.div
        ...
        {categories.map((cat, idx) => (
          ...
          >
            <Link href={`/collections/${cat.slug}`} className="group block">
              <div className="relative rounded-md h-80 overflow-hidden bg-charcoal-matte/80 border border-glass-border-subtle hover:border-accent-cyan/60 transition-colors duration-300 shadow-glass-md">
                <motion.img
                  src={
                    cat.image_url ||
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-obsidian-base via-obsidian-base/40 to-transparent opacity-90 group-hover:opacity-75 transition-opacity" />
